...
-- Step 1: Update any existing NULL values in multilingual columns FIRST
...
-- Step 2: Drop NOT NULL constraint from old columns
...
-- Step 3: Drop unique constraints from old columns
ALTER TABLE public.products
...
-- Step 4: Make multilingual columns NOT NULL instead
ALTER TABLE public.products
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'products_slug_es_key'
  ) THEN
    ALTER TABLE public.products
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'products_slug_en_key'
  ) THEN
    ALTER TABLE public.products
