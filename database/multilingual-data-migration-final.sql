...
-- ================================================
-- 3. CREAR ETIQUETAS DE BLOG EN INGLÉS (PRIMERO)
-- ================================================

-- Crear etiquetas de blog solo si no existen
INSERT INTO public.blog_tags (name_es, name_en, slug_es, slug_en, created_at)
SELECT * FROM (VALUES
...
) AS new_tags(name_es, name_en, slug_es, slug_en, created_at)
WHERE NOT EXISTS (
  SELECT 1 FROM public.blog_tags WHERE slug_es = new_tags.slug_es
);

-- ================================================
-- 4. CREAR POSTS DE BLOG CON CONTENIDO EN INGLÉS
-- ================================================

-- Primero verificar si ya existen posts de blog
-- Si no existen, crear posts usando un usuario existente o crear uno temporal

-- Crear posts de blog de ejemplo en inglés
-- Usar COALESCE para manejar el caso donde no hay usuarios
INSERT INTO public.blog_posts (
  title, -- Campo requerido original
  slug, -- Campo requerido original
  excerpt,
  content,
  featured_image,
  status,
...
(
  'Guía Completa para el Cuidado de Joyería de Oro', -- title (requerido)
  'guia-cuidado-joyeria-oro', -- slug (requerido)
...
Perfumes, lociones y productos de limpieza pueden dañar el oro. Siempre ponte la joyería después de aplicar estos productos.',
  ...
  NOW(),
  NOW(),
  COALESCE((SELECT id FROM public.profiles LIMIT 1), gen_random_uuid()), -- Usar usuario existente o crear UUID temporal
  -- Campos multilingües
...
(
  'Tendencias en Joyería para 2025', -- title (requerido)
  'tendencias-joyeria-2025', -- slug (requerido)
...
-- ================================================
-- 5. ASOCIAR ETIQUETAS CON POSTS DE BLOG
-- ================================================
...
-- Verificar estructura de la tabla blog_posts
SELECT 
  column_name,
  data_type,
  is_nullable
